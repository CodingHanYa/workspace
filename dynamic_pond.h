...
public:
	...
	*/
	void addThreads(int tnumb = 1) // fixme: what if submit(addThread)
	{
		int idx = 0;
		assert(tnumb >= 0);
		while (tnumb--)
		{
			...
			if (!deleted_threads.empty())
			{
				...
				pond[idx].join();  // todo: remove this
			...
			else
			{
				shared_locker.unlock();
				int idx = pond.size();
				pond.emplace_back(&DynamicThreadPond::worker, this, idx);
			}
			thread_numb++;
		}
	}
	...
	*/
	...
	void waitForTasks() // fixme: what if submit(waitForTasks)
	{
		waiting = true;
		HipeUniqGuard locker(shared_locker);
...
private:
	...
	void worker(int index)
	{
		...
		while (!stop)
		{
			...
			{
				shrink_numb--;
				deleted_threads.push(index); // todo: use recursive mutex
				break;
			}
			if (!stop)
			{
				...
				{
					HipeUniqGuard locker(shared_locker);
					task_done_cv.notify_one(); // fixme: what if many thread wait tasks done ? use notify all
				}
			}
		}
	}
