...
public:
	...
	void bindHandle(AutoThread&& handler)
	{
		this->handle = std::move(handler);
	}
...
public:
	...
	*/
	...
	void submitInBatch(_Container&& container, uint size) // todo: use range based for loop
	{
		if (thread_cap)
		{
	...
	void setRefuseCallBack(F&& foo, _Argv&& ... argv)
	{
		if (!thread_cap)
		{
			throw std::logic_error(
					"The refuse callback will never be invoked because the capacity has been setted unlimited");
		}
		else
		{
